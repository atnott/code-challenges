...
std::string Moon::get_name_file_by_year() {
    char buffer[64];
    std::snprintf(buffer, sizeof(buffer), "Moon/Moon%04d.dat", dt.get_year());
    return buffer;
}

void Moon::calculate_events() {
...
void Moon::print_results(std::string rise, std::string set, std::string culm_time, double max_el) {
